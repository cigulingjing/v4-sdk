...
contract ChainXAuctionV2 is ReentrancyGuard, Ownable {
    ...
    event LockRevealed(uint256 indexed auctionId, bytes32 indexed lockId, uint256 revealTime);
    event BidPlaced(uint256 indexed auctionId, bytes32 indexed lockId, address indexed bidder, uint256 depositAmount);
    event BidRevealed(uint256 indexed auctionId, address indexed bidder, uint256 bidRevealValue);
    event MatchResultSubmitted(uint256 indexed auctionId, bytes32 indexed lockId, address indexed bidder, uint256 bidValue);
    event MatchResultChallenged(uint256 indexed auctionId, bytes32 indexed lockId, address indexed challenger, string reason);
    event MatchResultWithdrawn(uint256 indexed auctionId, bytes32 indexed lockId, address indexed recipient, uint256 amount);
    event LockCreated(uint256 indexed auctionId, bytes32 indexed lockId, uint256 revealTime);
    event BidUpdated(uint256 indexed auctionId, bytes32 indexed lockId, address indexed bidder);
    event VaultAddressUpdated(address indexed oldVault, address indexed newVault);

    // ============ 拍卖核心功能 ============
    constructor(address initialVaultAddress) ReentrancyGuard() Ownable(msg.sender) {
    ...
    function createAuction(bytes memory crossChainMessage) external {
        ...
        // 解析原始交易
        TransactionParser.RawTransaction memory rawTx = TransactionParser.parseRawTransaction(ccData.rawTransaction);
        TransactionParser.RecptLog memory log = TransactionParser.parseRecptLog(ccData.rawRecpt);
        ...
        (auctionType, activeAuctionCount, revealStartTime) = abi.decode(log.eventData, (uint32, uint256, uint256));
        ...
        // ! 默认的发送地址
        // sender = address(0x5F40D0aA63957D41ec56500c5dc61c28Ae866a5e);
        console.log("sender address:", sender);
        
        uint256 auctionCheckId = uint256(keccak256(abi.encodePacked(
            sender,
        ...
        auctions[auctionId].revealTime = revealStartTime + REVEAL_PERIOD_SECRECT;
        auctions[auctionId].isActive = true;
        auctions[auctionId].challengeEndTime = revealStartTime + REVEAL_PERIOD_SECRECT + REVEAL_PERIOD_BID + CHALLENGE_PERIOD;
        
        emit AuctionCreated(auctionId, auctionType, auctions[auctionId].revealTime);
    }

    // 调试接口：根据 rawTx 计算恢复的签名者
    function debugRecover(bytes memory rawTransaction) external pure returns (address signer, bytes32 txHash, uint256 chainIdSig, uint8 vAdj, uint256 nonce, uint256 gasPrice, uint256 gasLimit, uint256 value) {
        TransactionParser.RawTransaction memory rawTx = TransactionParser.parseRawTransaction(rawTransaction);
    ...
    function placeBid(uint256 auctionId, address seller, bytes32 vaultSecretHash, bytes32 bidSecretHash) external payable {
        Auction storage auction = auctions[auctionId];
        require(auction.isActive, "Auction not active");
        require(block.timestamp < auction.revealTime, "Auction ended");
        
        // auction可以支持多个vault竞标，每个vault对应一个lockId
        bytes32 targetLockId = keccak256(abi.encodePacked(
            auction.chainid,
            vaultAddress,
            auctionId,
            seller,
        ...
        if (auction.auctionBid[msg.sender].revealTime == 0) {  
            ...
            newBid.revealTime = auction.revealTime + REVEAL_PERIOD_SECRECT + REVEAL_PERIOD_BID;
    ...
    function revealLock(uint256 auctionId, bytes32 lockId, uint256 value, bytes32 salt) external {
        Auction storage auction = auctions[auctionId];
        require(auction.isActive, "Auction not active");
        require(block.timestamp < auction.revealTime, "Auction ended");
        ...
        // 必须限定卖方才可以揭示。
        require(lockInfo.seller == msg.sender, "Lock is not for the current user");
        require(lockInfo.revealTime == 0, "Lock already revealed");
        require(keccak256(abi.encodePacked(value,salt))==lockInfo.vaultSecretHash, "Invalid lock reveal, hash secret is not match");
    ...
    function revealBid(uint256 auctionId, uint256 value, bytes32 salt) external {
        ...
        if (bid.isSealed) {
            ...
            require(keccak256(abi.encodePacked(value, salt)) == bid.bidSecretHash, "Invalid bid reveal");
        }
        bid.bidValue = value;
    ...
    function claimBidDepositAfterReveal(uint256 auctionId) external {
        Auction storage auction = auctions[auctionId];
        require(block.timestamp > auction.revealTime, "Reveal period not ended");
        ...
        for(uint256 i = 0; i < bid.targetVaults.length; i++) {
            ...
            require(auction.vaults[vaultId].salt != bytes32(0), "Vault is not revealed");
        }
    ...
    function claimBidDepositAfterChallenge(uint256 auctionId) external {
        Auction storage auction = auctions[auctionId];
        require(block.timestamp > auction.challengeEndTime, "Challenge period not ended");
        // 判断bid可提取，未竞价成功
        require(auction.auctionMatches[msg.sender].isChallenged || auction.auctionMatches[msg.sender].vaultId == bytes32(0), "Bid is challenged or not win the auction");
    ...
    function claimBidDepositAfterWithdraw(uint256 auctionId) external {
        Auction storage auction = auctions[auctionId];
        require(block.timestamp > auction.challengeEndTime + WITHDRAW_PERIOD, "Withdraw period not ended");
    ...
    function submitMatchResults(
        ...
        require(lockIds.length == bidders.length && bidders.length == finalValues.length, "Length mismatch");
        
        Auction storage auction = auctions[auctionId];
        require(lockIds.length == auction.vaultCount, "Count mismatch");
        require(auction.isActive, "Auction not active");
        require(block.timestamp >= auction.revealTime + REVEAL_PERIOD_BID, "Bid reveal period not ended");
        ...
        for (uint256 i = 0; i < lockIds.length; i++) {
            ...
            require(auction.vaults[lockId].revealTime == 0, "Lock not revealed");
    ...
    function challengeMatchResult(
        uint256 auctionId, 
        address bidder
    ) external {
        Auction storage auction = auctions[auctionId];
        require(auction.isActive, "Auction not active");
        require(block.timestamp < auction.challengeEndTime, "Challenge period ended");
    ...
    // ============ 查询功能 ============
    function getLockInfo(uint256 auctionId, bytes32 lockId) external view returns (bool exists, uint256 revealTime, bytes32 secret) {
    ...
    function getMatchResult(
        ...
        MatchResult storage matching = auctions[auctionId].auctionMatches[bidder];
        return (
            matching.vaultId,
            matching.isChallenged,
            matching.isWithdrawn
        );
    }
